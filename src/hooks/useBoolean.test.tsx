...
describe('useBoolean', () => {
    ...
    it.each([
        ...
        'calls %s to update the value to %s',
        (callback, expectedValue, initialValue) => {
            const { result } = renderHook(() =>
                useBoolean(initialValue as boolean)
            );
            act(() =>
                result.current[1][callback as keyof UseBooleanCallbacks]()
            );
