...
describe('Progress', () => {
    test('successPercent should decide the progress status when it exists', () => {
        ...
        expect(
            wrapper.querySelectorAll('.progress-status-success')
        ).toHaveLength(0);
    ...
    test('get correct line-gradient', () => {
        expect(
            handleGradient({ from: 'test', to: 'test' }).backgroundImage
        ).toBe('linear-gradient(to right, test, test)');
        expect(handleGradient({}).backgroundImage).toBe(
            'linear-gradient(to right, var(--primary-color), var(--primary-color))'
        );
        expect(
            handleGradient({ from: 'test', to: 'test', '0%': 'test' })
                .backgroundImage
    ...
    test('should show success status when status is invalid', () => {
        ...
        const { container: wrapper } = render(
            <Progress
                percent={100}
                status={'invalid' as ProgressProps['status']}
            />
        );
        expect(
            wrapper.querySelectorAll('.progress-status-success')
    ...
    test('steps should be changable', () => {
        ...
        expect(
            wrapper.querySelectorAll('.progress-steps-item-active').length
        ).toBe(3);
    ...
    test('steps should be changable when has strokeColor', () => {
        ...
        expect(
            wrapper.querySelectorAll<HTMLDivElement>('.progress-steps-item')[0]
                .style.backgroundColor
        ).toBe('rgb(24, 144, 255)');
        rerender(<Progress steps={5} percent={40} strokeColor="#1890ff" />);
        expect(
            wrapper.querySelectorAll<HTMLDivElement>('.progress-steps-item')[2]
                .style.backgroundColor
        ).toBe('');
        expect(
            wrapper.querySelectorAll<HTMLDivElement>('.progress-steps-item')[1]
                .style.backgroundColor
        ).toBe('rgb(24, 144, 255)');
    });
