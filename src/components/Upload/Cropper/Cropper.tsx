                ...
                beforeUpload: (file: OcFile, fileListArgs: OcFile[]) => {
                    return new Promise(async (resolve, reject) => {
                        if (cb.current.beforeCrop) {
                            const shouldCrop: boolean =
                                await cb.current.beforeCrop(file, fileListArgs);
                            if (!shouldCrop) {
                                return reject();
                            }
                        }
    ...
    const onOk = useCallback(async (): Promise<void> => {
        ...
        if (rotate && easyCropRef.current.rotateVal !== INIT_ROTATE) {
            const { naturalWidth: imgWidth, naturalHeight: imgHeight } =
                imgSource;
            const angle: number =
                easyCropRef.current.rotateVal * (Math.PI / 180);
        ...
        canvas.toBlob(
            async (blob: Blob | null) => {
                ...
                const result = await beforeUploadRef.current(newFile, [
                    newFile,
                ]);
                ...
                if (result === false) {
                    return rejectRef.current(
                        new Error('beforeUpload returned false')
                    );
                }
