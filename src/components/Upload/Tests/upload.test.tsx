...
describe('Upload', () => {
  ...
  test('correct drop class names when type is drop', () => {
    ...
    expect(wrapper.querySelectorAll('.upload-drop-uploading').length).toBe(1);
  });
  ...
  test('Proxy should support deepClone', async () => {
    ...
    ['uid', 'name', 'lastModified', 'lastModifiedDate', 'size', 'type'].forEach(
      (key) => {
  ...
  test('<Upload /> should pass <UploadList />', async () => {
    ...
    expect(wrapper.querySelectorAll('.upload-list').length).toBeGreaterThan(0);
  });
