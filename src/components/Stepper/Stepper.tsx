...
export const Stepper: FC<StepperProps> = React.forwardRef(
    (props: StepperProps, ref: Ref<HTMLDivElement>) => {
        ...
        const [completeAriaLabelText, setCompleteAriaLabelText] =
            useState<string>(defaultCompleteAriaLabelText);
        ...
        const [scrollUpAriaLabelText, setScrollUpAriaLabelText] =
            useState<string>(defaultScrollUpAriaLabelText);
        ...
        const handleScroll = (): void => {
            ...
            if (layout === 'horizontal') {
                setNextDisabled(
                    htmlDir === 'rtl'
                        ? steps?.scrollLeft ===
                              steps?.offsetWidth - steps?.scrollWidth
                        : steps?.scrollLeft ===
                              steps?.scrollWidth - steps?.offsetWidth
                );
                setPreviousDisabled(steps?.scrollLeft === 0);
            } else {
                setNextDisabled(
                    steps?.scrollTop ===
                        steps?.scrollHeight - steps?.offsetHeight
                );
        ...
        const primaryButton = (
            ...
            <PrimaryButton
                ...
                text={complete ? null : (index + 1).toString()}
            />
        );

        const secondaryButton = (index: number): JSX.Element => (
            <SecondaryButton
                ariaLabel={(index + 1).toString()}
