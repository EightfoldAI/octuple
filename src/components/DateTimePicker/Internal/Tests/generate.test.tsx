    ...
    list.forEach(({ name, generateConfig }) => {
        describe(name, () => {
            it('get', () => {
                ...
                const fixedDate = generateConfig.getFixedDate(
                    '1990-09-03 01:02:03'
                );
                ...
                describe('parse', () => {
                    it('basic', () => {
                        ['2000-01-02', '02/01/2000'].forEach((str) => {
                            const date = generateConfig.locale.parse(
                                'en_US',
                                str,
            ...
            it('getWeekFirstDay', () => {
                expect(generateConfig.locale.getWeekFirstDay('en_US')).toEqual(
                    0
                );
                expect(generateConfig.locale.getWeekFirstDay('zh_CN')).toEqual(
                    1
                );
            ...
            it('getWeekFirstDate', () => {
                const formatStr =
                    name === 'date-fns' ? 'yyyy-MM-dd' : 'YYYY-MM-DD';
                const usDate = generateConfig.locale.getWeekFirstDate(
                    'en_US',
                    generateConfig.locale.parse('en_US', '2020-12-30', [
                        formatStr,
                    ])
                );
                const cnDate = generateConfig.locale.getWeekFirstDate(
                    'zh_CN',
                    generateConfig.locale.parse('zh_CN', '2020-12-30', [
                        formatStr,
                    ])
                );
                expect(
                    generateConfig.locale.format('en_US', usDate, formatStr)
                ).toEqual('2020-12-27');
                expect(
                    generateConfig.locale.format('zh_CN', cnDate, formatStr)
                ).toEqual('2020-12-28');
            });
            ...
            it('getShortWeekDays', () => {
                expect(
                    generateConfig.locale.getShortWeekDays!('zh_CN')
                ).toEqual(['日', '一', '二', '三', '四', '五', '六']);
                expect(
                    generateConfig.locale.getShortWeekDays!('en_US')
