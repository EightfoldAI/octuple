import { DECADE_UNIT_DIFF } from '../Partials/DecadePartial/Decade.types';
import type {
    PartialMode,
    NullableDateType,
    OcPickerMode,
...
export function isSameDecade<DateType>(
    ...
    const equal: boolean = isNullEqual(decade1, decade2);
...
export function isSameYear<DateType>(
    ...
    const equal: boolean = isNullEqual(year1, year2);
    if (typeof equal === 'boolean') {
        return equal;
    }
...
export function getQuarter<DateType>(
    ...
    const quota: number = Math.floor(generateConfig.getMonth(date) / 3);
    return quota + 1;
}
...
export function isSameDate<DateType>(
    ...
    const equal: boolean = isNullEqual(date1, date2);
    if (typeof equal === 'boolean') {
        return equal;
    }
...
export function getWeekStartDate<DateType>(
    ...
    const weekFirstDay: number = generateConfig.locale.getWeekFirstDay(locale);
    const monthStartDate: DateType = generateConfig.setDate(value, 1);
    const startDateWeekDay: number = generateConfig.getWeekDay(monthStartDate);

    let alignStartDate: DateType = generateConfig.addDate(
        monthStartDate,
...
export function parseValue<DateType>(
    ...
    return generateConfig.locale.parse(
        locale.locale,
        value,
        formatList as string[]
    );
}
...
export function getCellDateDisabled<DateType>({
    ...
    switch (mode) {
        ...
        case 'month': {
            ...
            const endDate: number = generateConfig.getDate(
        ...
        case 'quarter': {
            const startMonth: number =
                Math.floor(generateConfig.getMonth(cellDate) / 3) * 3;
            const endMonth: number = startMonth + 2;
        ...
        case 'decade': {
            const year: number = generateConfig.getYear(cellDate);
            const startYear: number =
                Math.floor(year / DECADE_UNIT_DIFF) * DECADE_UNIT_DIFF;
            const endYear: number = startYear + DECADE_UNIT_DIFF - 1;
