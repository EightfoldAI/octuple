...
export const Slide: FC<CarouselSlideProps> = React.forwardRef(
    (props: CarouselSlideProps, ref: Ref<HTMLDivElement>) => {
        ...
        const forkedRef: (node: any) => void = useForkedRef(
            ref,
            carouselSlideRef
        );
        ...
        useEffect((): void => {
            if (active) {
                ...
                if (count !== 0)
                    setOrderClassName(
                        (styles as any)[`carousel-slide-${direction}`]
                    );
            }
        ...
        useEffect(() => {
            carouselSlideRef.current?.addEventListener(
                'transitionstart',
                () => {
            ...
            return () => {
                carouselSlideRef.current?.removeEventListener(
                    'transitionstart',
                    () => {
                        active && setAnimating(true);
                    }
