  ...
  (
    ...
    useEffect(() => {
      const updatedOptions = options.map((opt) => ({
        ...opt,
        selected: (defaultValue && opt.value === defaultValue) || opt.selected,
      }));
    ...
    const onInputChange = (
      ...
      if (value) {
        ...
        setSearchQuery('');
        setOptions(options.map((option) => ({ ...option, hideOption: false })));
      }
    };
    ...
    const showPills = () => {
      ...
      return (
        selectedCount !== 0 && !(filterable && dropdownVisible) && multiple
      );
    };
    ...
    return (
      ...
      >
        ...
        <Dropdown
          {...dropdownProps}
          onVisibleChange={(isVisible) => onDropdownVisibilityChange(isVisible)}
