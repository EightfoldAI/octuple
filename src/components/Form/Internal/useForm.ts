...
export class FormStore {
  ...
  private getFieldListValues = (
    ...
    fieldEntities.forEach((entity: OcFieldEntity | InvalidateFieldEntity) => {
      const namePath =
      ...
      if (!filterFunc) {
        ...
        const meta: OcMeta = 'getMeta' in entity ? entity.getMeta() : null;
        if (filterFunc(meta)) {
          filteredNameList.push(namePath);
        }
      }
  ...
  private getFields = (): InternalOcFieldData[] => {
    ...
    const fields = entities.map((field: OcFieldEntity): InternalOcFieldData => {
      const namePath = field.getNamePath();
  ...
  private validateFields: InternalOcValidateFields = (
    ...
    this.getFieldEntities(true).forEach((field: OcFieldEntity) => {
      ...
      if (options?.recursive && provideNameList) {
        ...
        if (
          ...
          namePath.every(
            (nameUnit, i) =>
              nameList[i] === nameUnit || nameList[i] === undefined
          )
        ) {
          namePathList.push(namePath);
        }
    ...
    const returnPromise: Promise<OcStore | OcValidateErrorEntity | string[]> =
      summaryPromise
        .then((): Promise<OcStore | string[]> => {
          if (this.lastValidatePromise === summaryPromise) {
            return Promise.resolve(this.getFieldListValues(namePathList));
          }
          return Promise.reject<string[]>([]);
        })
        .catch((results: { name: InternalOcNamePath; errors: string[] }[]) => {
