  ...
  const listContext = useMemo<OcListContextProps>(
    () => ({
      getKey: (namePath: InternalOcNamePath) => {
        ...
        return [(keyManager.keys as any)[pathName], namePath.slice(len + 1)];
      },
    }),
    [prefixName]
  );
      ...
      <OcFieldContext.Provider value={fieldContext}>
        ...
        >
          {({ value = [], onChange }, meta) => {
            ...
            const getNewValue = () => {
              const values = getFieldValue(prefixName || []) as OcStoreValue[];
              return values || [];
            };
            /**
            ...
            return children(
              (listValue as OcStoreValue[]).map((__, index): OcListField => {
