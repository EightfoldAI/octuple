...
describe('legacy.switch-field', () => {
    ...
    it('Preserve right fields when switch them', async () => {
        ...
        expect(Object.keys(form.getFieldsValue())).toEqual(
            expect.arrayContaining(['a'])
        );
        expect(form.getFieldValue('a')).toBe('value1');
        expect(wrapper.find('.one').last().getDOMNode().value).toBe('value1');
        ...
        expect(form.getFieldValue('a')).toBe('value1');
        expect(wrapper.find('.two').last().getDOMNode().value).toBe('value1');
    });
});
