...
describe('OcForm.InitialValues', () => {
  test('works', () => {
    ...
    expect(form.getFieldListValues(['username', ['path1', 'path2']])).toEqual({
      username: 'Lola',
      path1: {
        path2: 'Mia',
      },
    });
    expect(getField(wrapper, 'username').find('input').props().value).toEqual(
      'Lola'
    );
    expect(
    ...
    const Demo = () => {
      ...
      return (
        <>
          ...
          {show && (
            <OcForm form={form} initialValues={formValue} preserve={false}>
