...
describe('Form', () => {
  ...
  describe('noStyle Form.Item', () => {
    test('work', async () => {
      ...
      expect(container.querySelectorAll('.form-item-with-help')).toBeTruthy();
      expect(container.querySelectorAll('.form-item-has-error')).toBeTruthy();
    ...
    test.skip('should clean up', async () => {
      ...
      expect(container.querySelector('.form-item-explain').textContent).toEqual(
        'aaa'
      );
    ...
    function App() {
      ...
      return (
        <Form>
          ...
          <PrimaryButton onClick={() => updateMessage('mia')} text={'Update'} />
        </Form>
      );
    }
  ...
  test('validation message should has alert role', async () => {
    ...
    await sleep(100);
    expect(
      wrapper.find('.form-item-explain div').getDOMNode().getAttribute('role')
    ).toBe('alert');
  });
  ...
  test.skip('noStyle should not affect status', () => {
    ...
    expect(container.querySelector('.custom-select-b')?.className).toContain(
      'status-error'
    );
    expect(container.querySelector('.custom-select-b')?.className).toContain(
      'in-form-item'
    );
    expect(container.querySelector('.custom-select-c')?.className).toContain(
      'status-error'
    );
    expect(container.querySelector('.custom-select-c')?.className).toContain(
      'in-form-item'
    );
    expect(container.querySelector('.custom-select-d')?.className).toContain(
      'status-warning'
    );
    expect(container.querySelector('.custom-select-d')?.className).toContain(
      'in-form-item'
  ...
  test('should not affect Surface UI children styles', () => {
    ...
    expect(container.querySelector('.modal-input')?.className).not.toContain(
      'in-form-item'
    );
    expect(container.querySelector('.modal-input')?.className).not.toContain(
      'status-error'
      ...
      return (
        <Form form={form} name="my-form">
          <Form.Item name="required" rules={[{ required: true }]}>
            <CustomInput classNames="custom-input-required" value="" />
          </Form.Item>
