...
describe('Util', () => {
    ...
    describe('convertDataToEntities', () => {
        it('basic convert', () => {
            ...
            // Used for `cascader` to get fully path
        ...
        it('with childrenPropName', () => {
            ...
            expect(entities.keyEntities.light.children).toEqual([
                entities.keyEntities.bamboo,
            ]);
            expect(entities.keyEntities.bamboo.parent).toBe(
                entities.keyEntities.light
            );
        });
    });
    ...
    describe('conductCheck', () => {
        describe('basic', () => {
            ...
            it('uncheck', () => {
                ...
                const removedKeys = allCheckedKeys.filter(
                    (key) => key !== 'spoon'
                );
                const { checkedKeys, halfCheckedKeys } = conductCheck(
                    removedKeys,
        ...
        describe('part disabled', () => {
            ...
            it('check', () => {
                ...
                expect(result1.checkedKeys.sort()).toEqual(
                    ['not', 'it'].sort()
                );
            ...
            it('uncheck', () => {
                ...
                const result2 = conductCheck(
                    allCheckedKeys.filter(
                        (key) => key !== 'to' && key !== 'be'
                    ),
                    { checked: false, halfCheckedKeys: [] },
                    keyEntities
                );
                expect(result2.checkedKeys.sort()).toEqual(
                    ['war', 'are', 'not', 'it'].sort()
                );
                expect(result2.halfCheckedKeys.sort()).toEqual([].sort());
            });
