...
const NodeList = React.forwardRef<NodeListRef, NodeListProps<any>>(
    (props, ref) => {
        ...
        const [motionType, setMotionType] = React.useState<
            'show' | 'hide' | null
        >(null);
        ...
        return (
            <>
                ...
                >
                    <div className={styles.treeIndent}>
                        <div
                            ...
                            className={styles.treeIndentUnit}
                        />
                    </div>
                </div>
                ...
                <VirtualList<FlattenNode>
                    ...
                    ref={listRef}
                    onVisibleChange={(
                        originList: Iterable<unknown>,
                        fullList: any[]
                    ) => {
                        const originSet = new Set(originList);
                        const restList = fullList.filter(
                            (item) => !originSet.has(item)
                        );

                        // Motion node is not render. Skip motion
                        if (
                            restList.some(
                                (item) => itemKey(item) === MOTION_KEY
                            )
                        ) {
                            onMotionEnd();
                        }
                    ...
                    {(treeNode: {
                        ...
                        return (
                            <MotionTreeNode
                                ...
                                motion={motion}
                                motionNodes={
                                    key === MOTION_KEY ? transitionRange : null
                                }
