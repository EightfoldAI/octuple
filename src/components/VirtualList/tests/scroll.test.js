...
describe('List.Scroll', () => {
  ...
  it('scrollTo null will show the scrollbar', () => {
    ...
    expect(
      wrapper.find('.virtual-list-scrollbar-thumb').props().style.display
    ).not.toEqual('none');
    jest.useRealTimers();
  });
  ...
  describe('scrollbar', () => {
    it('moving', () => {
      ...
      // Mouse down
      wrapper.find('.virtual-list-scrollbar-thumb').simulate('mousedown', {
        pageY: 0,
      });
      ...
      expect(
        wrapper.find('.virtual-list-holder').props().style.pointerEvents
      ).toEqual('none');
    ...
    describe('not show scrollbar when disabled virtual', () => {
      [
        ...
        it(name, () => {
          ...
          expect(wrapper.find('.virtual-list-scrollbar-thumb')).toHaveLength(0);
        });
      });
    });
  });
  ...
  it('no bubble', () => {
    ...
    wrapper.find('.virtual-list-scrollbar').simulate('mousedown', {
      preventDefault,
      stopPropagation,
    });
  ...
  it('onScroll should trigger on correct target', () => {
    ...
    wrapper.find('.virtual-list-holder').simulate('scroll');

    expect(currentTarget).toBe(
      wrapper.find('.virtual-list-holder').hostNodes().instance()
    );
  });
